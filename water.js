...
void main()
{
	...
	vec3 position = vec3(2.0 * position_in - 1.0);
	position.z = 0.0;
	float terrainHeight = texture(uSamplerTerrain, position_in.xy).r;
...
void main()
{
	...
	float r = mix(0.0, 1.0, (reflection.r + refraction.r)/2.0);
	float g = mix(0.0, 1.0, (reflection.g + refraction.g)/2.0);
	float b = mix(0.0, 1.0, (reflection.b + refraction.b)/2.0);
...
function buildTerrainMesh() {
	...
	// Store x, y and z postion of each point
	let data_position = new Float32Array(iMax * jMax * 3);
	for (let j = 0; j < jMax; j++) {
		for (let i = 0; i < iMax; i++) {
			// x
			data_position[3 * (i + j * iMax)] = i / (iMax - 1);
			// y
			data_position[3 * (i + j * iMax) + 1] = j / (jMax - 1);
			// z
			data_position[3 * (i + j * iMax) + 2] = 0.0;
		}
	}
